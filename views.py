...
def product_detail(request, product_id, page=0, template_name='fprice/product_detail.html', **kwargs):
    ...
    price_list = Price.objects.raw('SELECT * FROM (SELECT * FROM (SELECT shop_id AS sid, MAX(last_time_update) as maxtime FROM fprice_price GROUP BY product_id, shop_id HAVING product_id = %s) as fmax LEFT JOIN fprice_price as fp ON fmax.sid = fp.shop_id AND fmax.maxtime = fp.last_time_update) as ffull LEFT JOIN fprice_shop AS fs ON ffull.shop_id = fs.id', [price_param])
    return list_detail.object_list(
        request,
...
def shop_detail(request, shop_id, page=0, template_name='fprice/shop_detail.html', **kwargs):
    ...
    price_param = shop_id
    price_list = Price.objects.raw('SELECT * FROM (SELECT * FROM (SELECT product_id AS pid, MAX(last_time_update) as maxtime FROM fprice_price GROUP BY product_id, shop_id HAVING shop_id = %s) as fmax LEFT JOIN fprice_price as fp ON fmax.pid = fp.product_id AND fmax.maxtime = fp.last_time_update) as ffull LEFT JOIN fprice_product AS fs ON ffull.product_id = fs.id', [price_param])
    return list_detail.object_list(
        request,
